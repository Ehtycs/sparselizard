...
void indexmat::errorifempty(void)
{
    if (numrows*numcols == 0)
    {
        ...
        tmp  << "Error in 'indexmat' object: cannot perform operation on empty matrix" << std::endl;
...
indexmat::indexmat(long long int numberofrows, long long int numberofcolumns, std::vector<int> valvec)
{
    ...
    {
        std::stringstream tmp;
        tmp  << "Error in 'indexmat' object: expected a value vector of size " << numberofrows*numberofcolumns << std::endl;
        throw std::runtime_error(tmp.str());
    }
...
indexmat::indexmat(std::vector<indexmat> input)
{
    ...
    for (long long int i = 1; i < input.size(); i++)
    {
        ...
        {
            std::stringstream tmp;
            tmp  << "Error in 'indexmat' object: dimension mismatch in concatenation" << std::endl;
            throw std::runtime_error(tmp.str());
        }
    }
