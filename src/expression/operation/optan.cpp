...
std::vector<std::vector<densemat>> optan::interpolate(elementselector& elemselect, std::vector<double>& evaluationcoordinates, expression* meshdeform)
{
    ...
    tmp  << "Error in 'optan' object: without FFT tan() can only be computed for constant (harmonic 1) operations" << std::endl;

    throw std::runtime_error(tmp.str());
}

densemat optan::multiharmonicinterpolate(int numtimeevals, elementselector& elemselect, std::vector<double>& evaluationcoordinates, expression* meshdeform)
{
    // Get the value from the universe if available and reuse is enabled:
    if (reuse && universe::isreuseallowed)
    {
...
std::shared_ptr<operation> optan::simplify(std::vector<int> disjregs)
{
    myarg = myarg->simplify(disjregs);
    
    if (myarg->isconstant())
        return std::shared_ptr<operation>(new opconstant(std::tan(myarg->getvalue())));
    else
        return shared_from_this();
}

std::shared_ptr<operation> optan::copy(void)
{
    std::shared_ptr<optan> op(new optan(myarg));
    *op = *this;
    op->reuse = false;
    return op;
}
