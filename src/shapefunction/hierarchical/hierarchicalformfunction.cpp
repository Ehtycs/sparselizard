...
int hierarchicalformfunction::gettypenumber(std::string fftypename)
{
    ...
    if (outtypenum == -1)
    {
        ...
        tmp  << "Error in 'hierarchicalformfunction' object: unknown type name '" << fftypename << "'" << std::endl;
...
std::string hierarchicalformfunction::gettypename(int fftypenumber)
{
    ...
    {
        std::stringstream tmp;
        tmp  << "Error in 'hierarchicalformfunction' object: unknown type number " << fftypenumber << std::endl;
        throw std::runtime_error(tmp.str());
    }
