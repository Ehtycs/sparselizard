...
void regiondefiner::defineskinregion(int regnum)
{
    ...
    if (physregdim == 0)
    {
        ...
        tmp  << "Error in 'regiondefiner' object: cannot get the skin of point elements" << std::endl;
...
void regiondefiner::defineexclusionregion(int regnum)
{
    ...
    for (int i = 0; i < toexclude[regnum].size(); i++)
    {
        ...
        {
            std::stringstream tmp;
            tmp  << "Error in 'regiondefiner' object: cannot exclude a " << curdim << "D region form a " << physregdim << "D region (dimensions must be equal)" << std::endl;
            throw std::runtime_error(tmp.str());
        }
    }
...
void regiondefiner::regionbox(int newphysreg, int selecteddim, std::vector<double> boxlimit, int physregtobox)
{
    ...
    {
        std::stringstream tmp;
        tmp  << "Error in 'regiondefiner' object: expected a vector of length 6 for the box limits {x1,x2,y1,y2,z1,z2}" << std::endl;
        throw std::runtime_error(tmp.str());
    }
    if (selecteddim > 3 || selecteddim < 0)
    {
        std::stringstream tmp;
        tmp  << "Error in 'regiondefiner' object: dimension of the elements to select cannot be " << selecteddim << std::endl;
        throw std::runtime_error(tmp.str());
    }
...
void regiondefiner::regionsphere(int newphysreg, int selecteddim, std::vector<double> centercoords, double radius, int physregtosphere)
{
    ...
    {
        std::stringstream tmp;
        tmp  << "Error in 'regiondefiner' object: expected a vector of length 3 for the sphere center {xc,yc,zc}" << std::endl;
        throw std::runtime_error(tmp.str());
    }
    if (selecteddim > 3 || selecteddim < 0)
    {
...
void regiondefiner::regionlayer(int newphysreg, int physregtoselectfrom, int physregtostartgrowth, int nl)
{
    ...
    {
        std::stringstream tmp;
        tmp  << "Error in 'regiondefiner' object: expected at least one layer to select" << std::endl;
        throw std::runtime_error(tmp.str());
    }
