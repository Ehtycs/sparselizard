...
int elements::add(int elementtypenumber, int curvatureorder, std::vector<int>& nodelist)
{
    ...
    if (mycurvatureorder != curvatureorder)
    {
        ...
        tmp  << "Error in 'elements' object: the mesh can contain only a single curvature order." << std::endl;
...
int elements::getdisjointregion(int elementtypenumber, int elementnumber, bool errorifnegative)
{
    ...
    else
    {
        ...
        std::stringstream tmp;
        tmp  << "Possible reason: too bad quality elements obtained when h-adapting a curved mesh (some identical edges might not have been merged and therefore AMR has failed)" << std::endl;
        throw std::runtime_error(tmp.str());
    }
}
...
std::vector<bool> elements::isflipped(int subelementtypenumber, std::vector<int>& subelementnumbers, int elementtypenumber, std::vector<int>& elementnumbers)
{
    ...
    for (int e = 0; e < numelems; e++)
    {
        ...
        {
            std::stringstream tmp;
            tmp  << "Error in 'elements' object: in 'isflipped' could not find " << sub.gettypename() <<  " " << cursub << " in " << parent.gettypename() << " " << curparent << std::endl;
            throw std::runtime_error(tmp.str());
        }
