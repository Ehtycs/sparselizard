...
void densemat::errorifempty(void)
{
    if (numrows*numcols == 0)
    {
        ...
        tmp  << "Error in 'densemat' object: cannot perform operation on empty matrix" << std::endl;
...
densemat::densemat(long long int numberofrows, long long int numberofcolumns, std::vector<double> valvec)
{
    ...
    {
        std::stringstream tmp;
        tmp  << "Error in 'densemat' object: expected a value vector of size " << numberofrows*numberofcolumns << std::endl;
        throw std::runtime_error(tmp.str());
    }
...
densemat::densemat(std::vector<densemat> input)
{
    ...
    for (long long int i = 1; i < input.size(); i++)
    {
        ...
        {
            std::stringstream tmp;
            tmp  << "Error in 'densemat' object: dimension mismatch in concatenation" << std::endl;
            throw std::runtime_error(tmp.str());
        }
    }
...
densemat densemat::multiply(densemat B)
{
    ...
    {
        std::stringstream tmp;
        tmp  << "Error in 'densemat' object: trying to multiply a " << numrowsA << "x" << numcolsA << " matrix by a "  << numrowsB << "x" << numcolsB << std::endl;
        throw std::runtime_error(tmp.str());
    }
...
densemat densemat::getinverse(void)
{
    ...
    {
        std::stringstream tmp;
        tmp  << "Error in 'densemat' object: can only invert a square densemat" << std::endl;
        throw std::runtime_error(tmp.str());
    }
