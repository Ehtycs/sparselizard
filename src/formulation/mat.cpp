...
void mat::errorifpointerisnull(void)
{
    if (rawmatptr == NULL)
    {
        ...
        tmp  << "Error in 'mat' object: cannot perform the operation (matrix is undefined)" << std::endl;
...
void mat::errorifinvalidated(void)
{
    ...
    {
        std::stringstream tmp;
        tmp  << "Error in 'mat' object: matrix cannot be used anymore (invalidated by hp-adaptivity)" << std::endl;
        throw std::runtime_error(tmp.str());
    }
}
